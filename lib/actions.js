...
export async function getBooks(query = '') {
    ...
    let queryBuilder = supabase.from('books').select('*').order('title');

    if (sanitizedQuery) {
        // Simple search across title, author, or isbn
        queryBuilder = queryBuilder.or(`title.ilike.%${sanitizedQuery}%,author.ilike.%${sanitizedQuery}%,isbn.ilike.%${sanitizedQuery}%`);
    }
